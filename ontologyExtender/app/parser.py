...
class OntoGroup:
    ...
    def __str__(self):
        return f'Group {self.id}: {self.facts}'
...
def get_xml(facts, url):
    ...
    for fact in facts:
        ...
        fact_element.set('FactID', str(fact.id))
        fact_element.set('LeadID', str(fact.id))
        for f in fact.facts:
...
"""
    print(get_xml(get_facts(resume), "\\shulga_tatyana_erikovna.txt"))
