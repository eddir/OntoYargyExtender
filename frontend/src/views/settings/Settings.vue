<template>
  <CContainer>
    <CRow>
      <CCol md="6">
        <CCard>
          <CCardHeader>OntoMuseum</CCardHeader>
          <CCardBody>
...
export default {
  ...
  created() {
    API.getVersion().then(response => this.version = response.data.response);
  },
  methods: {
    ping() {
      API.ping().then(response => Vue.$toast.success(response.data.response));
    }
  },
}
</script>
